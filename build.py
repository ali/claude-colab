...
def build_notebook():
    ...
    for cell in notebook['cells']:
        if cell['cell_type'] == 'code':
            ...
            source = source.replace('{{MARKDOWN_FORMATTER_HOOK}}', escape_for_python_string(markdown_formatter, use_triple_quotes=True))
            source = source.replace('{{STATUSLINE_HOOK}}', escape_for_python_string(statusline_script, use_triple_quotes=True))
            source = source.replace('{{UPDATE_DOCS_SCRIPT}}', escape_for_python_string(update_docs_script, use_triple_quotes=True))
